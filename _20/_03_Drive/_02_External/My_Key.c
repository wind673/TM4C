...
/* 全局结构体定义 -------------------------------------------------------------*/
Type_Key g_keyboard = {0};  //g_ 代表全局变量
...
void Key_Init(void)
{
	...
	/* Step 1 使能 ------------------------------*/       
	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOH);  //使能IO口     
	
	/* Step 2 配置成输入 ------------------------*/ 
  	GPIOPinTypeGPIOInput(GPIO_PORTH_BASE,GPIO_PIN_4|GPIO_PIN_5);   //配置成输入
	GPIODirModeSet(GPIO_PORTH_BASE, GPIO_PIN_4 | GPIO_PIN_5, GPIO_DIR_MODE_HW);
	
	/* Step 3 配置中断方式 ----------------------*/ 
    GPIOIntTypeSet(GPIO_PORTH_BASE,GPIO_PIN_4,GPIO_FALLING_EDGE);  //配置成下降沿 
	
	/* Step 4 配置中断IO口 ----------------------*/ 
    GPIOIntEnable(GPIO_PORTH_BASE,GPIO_PIN_4);   //配置中断的IO口  
	
	/* Step 5 使能中断 --------------------------*/ 
	IntEnable(INT_GPIOH);   //使能IO口
	IntMasterEnable();   
